...
class NeuralNetwork():
    ...
    def preprocess(self, TrainData):
        ...
        self.drange, self.atoms = self.get_dataset_info(TrainData)
    ...
    def get_dataset_info(self, data):
        ...
        -------
        _DRANGE: list
            List of min and max values of atomic descriptors.

        _ATOMS: list
            List of numbers of atoms per unit cell for all structures.
        """
        db = shelve.open(self.path+data)
        self.no_of_structures = len(list(db.keys()))
        self.no_of_descriptors = db['0']['x'].shape[1]
    ...
    def normalize(self, data, drange, unit, norm=[0., 1.]):
        ...
        if doit:
            ...
            path = self.path+'train_npys/'
            x_shape = (self.no_of_structures, max_atoms, self.no_of_descriptors)
            dx_shape = (self.no_of_structures, max_atoms, max_atoms, self.no_of_descriptors, 3)
            rdx_shape = (self.no_of_structures, max_atoms, self.no_of_descriptors, 6)
            force_shape = (self.no_of_structures, max_atoms, 3)

            APS, ENERGY, STRESS, GROUP = [], [], [], [] # !scale by atoms
            # Create memmap files
            X = np.memmap(path+'X.npy', dtype='float64', mode='w+', shape=x_shape)
            DX = np.memmap(path+'DXDR.npy', dtype='float64', mode='w+', shape=dx_shape)
            RDX = np.memmap(path+'RDXDR.npy', dtype='float64', mode='w+',shape=rdx_shape)
            FORCE = np.memmap(path+'FORCE.npy', dtype='float64', mode='w+', shape=force_shape)
            ...
            for i in range(self.no_of_structures):
                ...
                for element in self.elements:
                    ...
                    if e.size == 0:
                        ...
                        aps.append([e[0], e[-1]])
                        _x[e[0]:e[-1]+1] = norm[0] + np.einsum('j,ij->ij', scale, (data['x'][e[0]:e[-1]+1] - \
                                                     np.expand_dims(_drange[:, 0], 0)))
                        if unit == 'eV':
                ...
                # Open existing memmap files and write
                X = np.memmap(path+'X.npy', dtype='float64', mode='r+', shape=x_shape)
                DX = np.memmap(path+'DXDR.npy', dtype='float64', mode='r+', shape=dx_shape)
                RDX = np.memmap(path+'RDXDR.npy', dtype='float64', mode='r+',shape=rdx_shape)
                FORCE = np.memmap(path+'FORCE.npy', dtype='float64', mode='r+', shape=force_shape)
                X[i], DX[i], RDX[i], FORCE[i] = x, dxdr, rdxdr, force
            
            db.close()

            np.save(path+'APS.npy', APS)
            np.save(path+'ENERGY.npy', ENERGY)
            np.save(path+'STRESS.npy', STRESS)
...
class Dataset(data.Dataset):
    ...
    """
    def __init__(self, path, atoms, softmax, device, memory, fc, sc, batch, ndes, length):
        self.path = path+'train_npys/'
        self.atoms = atoms
        self.max_atoms = max(self.atoms)
        ...
        self.batch = batch
        self.length = length
        self.count = copy.deepcopy(batch)
        
        self.x_shape = (self.length, self.max_atoms, ndes)
        self.dx_shape = (self.length, self.max_atoms, self.max_atoms, ndes, 3)
        self.rdx_shape = (self.length, self.max_atoms, ndes, 6)
        self.force_shape = (self.length, self.max_atoms, 3)

        self.aps = np.load(self.path+'APS.npy')
        self.group = np.load(self.path+'GROUP.npy')
        self.energy = torch.from_numpy(np.load(self.path+'ENERGY.npy')).to(self.device)
        if self.sc:
            self.stress = torch.from_numpy(np.load(self.path+'STRESS.npy')).to(self.device)
        else:
            self.stress = [None]*self.length

        if memory == 'in':
            self.x, self.dxdr, self.rdxdr, self.force = [], [], [], []
            
            for i in range(self.length):
                X = np.memmap(self.path+'X.npy', dtype='float64', mode='r', shape=self.x_shape)
                x = copy.deepcopy(X[i][0:self.atoms[i]])
                self.x.append(torch.from_numpy(x).to(self.device))

                if self.fc:
                    DX = np.memmap(self.path+'DXDR.npy', dtype='float64', mode='r', shape=self.dx_shape)
                    dx = copy.deepcopy(DX[i][0:self.atoms[i], 0:self.atoms[i]])
                    self.dxdr.append(torch.from_numpy(dx).to(self.device))
                    FORCE = np.memmap(self.path+'FORCE.npy', dtype='float64', mode='r', shape=self.force_shape)
                    force = copy.deepcopy(FORCE[i][0:self.atoms[i]])
                    self.force.append(torch.from_numpy(force).to(self.device))
                else:
                    self.force.append(None)
                    self.dxdr.append(None)
                
                if self.sc and self.stress[i] is not None:
                    RDX = np.memmap(self.path+'RDXDR.npy', dtype='float64', mode='r',shape=self.rdx_shape)
                    rdx = copy.deepcopy(RDX[i][0:atoms[i]])
                    self.rdxdr.append(torch.from_numpy(rdx).to(self.device))
                else:
    ...
    def __getitem__(self, index):
        ...
        if self.memory == 'in':
            ...
            if self.count == self.batch:
                ...
                #except:
                #    pass
                self._X = np.memmap(self.path+'X.npy', dtype='float64', mode='r', shape=self.x_shape)
                if self.fc:
                    #try:
                    #    del self._DX
                    #    del self._FORCE
                    ...
                    #except:
                    #    pass
                    self._DX = np.memmap(self.path+'DXDR.npy', dtype='float64', mode='r', shape=self.dx_shape)
                    self._FORCE = np.memmap(self.path+'FORCE.npy', dtype='float64', mode='r', shape=self.force_shape)
                if self.sc:
                    #try:
                    #    del self._RDX
                    #    gc.collet()
                    #except:
                    #    pass
                    self._RDX = np.memmap(self.path+'RDXDR.npy', dtype='float64', mode='r', shape=self.rdx_shape)
                gc.collect()    

            #_dx = copy.deepcopy(self._DX[index][0:self.atoms[index], 0:self.atoms[index]])

            x = torch.from_numpy(self._X[index][0:self.atoms[index]]).to(self.device)
            if self.fc:
                #dxdr = torch.DoubleTensor(self._DX[index][0:self.atoms[index], 0:self.atoms[index]])
                dxdr = torch.from_numpy(self._DX[index][0:self.atoms[index], 0:self.atoms[index]])
                #dxdr = torch.from_numpy(_dx).to(self.device)
                force = torch.from_numpy(self._FORCE[index][0:self.atoms[index]]).to(self.device)
            else:
                dxdr = None
                force = None
            if self.sc and stress is not None:
                print(self._RDX[index]) #[0:atoms[index]])
                rdxdr = torch.from_numpy(self._RDX[index][0:atoms[index]]).to(self.device)
            else:
                rdxdr = None
            self.count += 1
