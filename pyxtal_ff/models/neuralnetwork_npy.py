...
class NeuralNetwork():
    ...
    def preprocess(self, TrainData):
        ...
        self.no_of_structures, self.no_of_descriptors, self.drange, self.atoms = \
                self.get_dataset_info(TrainData)
        ...
        self.data = data.DataLoader(Dataset(self.path, self.atoms, self.softmax, 
                                            self.device, self.memory, 
                                            self.force_coefficient, 
                                            self.stress_coefficient,
                                            self.no_of_structures),
    ...
    def calculate_loss(self, models, batch):
        ...
        for x, dxdr, rdxdr, energy, force, stress, sf, group, aps, atoms in batch:
            all_atoms += atoms
            _Energy = 0  # Predicted total energy for a structure
            _force = torch.zeros([atoms, 3], dtype=torch.float64, device=self.device)
            if self.stress_coefficient and (group in self.stress_group):
                _stress = torch.zeros([6], dtype=torch.float64, device=self.device)
            
            dedx, sdedx = {}, {}
            for i, (element, model) in enumerate(models.items()):
                if aps[i][1]-aps[i][0] > 0:
                    _x = x[aps[i][0]:aps[i][1]+1].requires_grad_()
                    _energy = model(_x).sum() # total energy for each specie
                    _Energy += _energy

                    if self.force_coefficient:
                        dedx[element] = torch.autograd.grad(_energy, _x, create_graph=True)[0]
                        _dxdr = dxdr[aps[i][0]:aps[i][1]+1]
                        _force += -1 * torch.einsum("ik, ijkl -> jl", dedx[element], _dxdr)

                    if self.stress_coefficient and (group in self.stress_group):
                        if self.force_coefficient is None:
                            dedx[element] = torch.autograd.grad(_energy, _x, create_graph=True)[0]
                        _rdxdr = rdxdr[aps[i][0]:aps[i][1]+1]
                        _stress += -1 * torch.einsum("ik, ikl->l", dedx[element], _rdxdr) # [6]
            
            energy_loss += sf.item()*((_Energy - energy) / atoms) ** 2
            energy_mae  += sf.item()*F.l1_loss(_Energy / atoms, energy / atoms)

            if self.force_coefficient:
                force_loss += sf.item()*self.force_coefficient * ((_force - force) ** 2).sum()
                force_mae  += sf.item()*F.l1_loss(_force, force) * atoms
    ...
    def get_dataset_info(self, data):
        """ Get relevant information about the dataset, including
        total structures, number of atoms per structure, descriptor range,
        ...
        no_of_descriptors = db['0']['x'].shape[1]
        ...
        if not os.path.exists(self.path+'drange.npy'):
            ...
            for i in range(no_of_structures):
                dic = db[str(i)]
                _ATOMS.append(len(dic['elements']))
            ...
            np.save(self.path+'atoms.npy', _ATOMS)

        else:
            _DRANGE = np.load(self.path+'drange.npy', allow_pickle=True)[0]
            _ATOMS = np.load(self.path+'atoms.npy')
            
        db.close()

        return no_of_structures, no_of_descriptors, _DRANGE, _ATOMS

    
    def normalize(self, data, drange, unit, norm=[0., 1.]):
        db = shelve.open(self.path+data)
        doit = True if not os.path.exists(self.path+'train_npys') else False
        bundle = 1000
        count_bundle = 1
        
        if doit:
            os.mkdir(self.path+'train_npys')
            max_atoms = max(self.atoms)

            X, DXDR, RDXDR, APS = [], [], [], []
            ENERGY, FORCES, STRESS, GROUP = [], [], [], []
            for i in range(self.no_of_structures):
                x = np.zeros([max_atoms, self.no_of_descriptors])
                dxdr = np.zeros([max_atoms, max_atoms, self.no_of_descriptors, 3])
                rdxdr = np.zeros([max_atoms, self.no_of_descriptors, 6])
                force = np.zeros([max_atoms, 3])
                aps = [] # atoms per species

                data = db[str(i)]
                _x = np.zeros_like(data['x'])
                _dx = np.zeros_like(data['dxdr'])
                _rdx = np.zeros_like(data['rdxdr'])
                shp = _x.shape

                for element in self.elements:
                    _drange = drange[element]
                    scale = (norm[1] - norm[0]) / (_drange[:, 1] - _drange[:, 0])
                    e = np.where(np.array(data['elements'])==element)[0]

                    if e.size == 0:
                        aps.append([0, 0])
                    elif e.size > 0:
                        aps.append([e[0], e[-1]])
                        _x[e[0]:e[-1]+1] = norm[0] + np.einsum('j,ij->ij', scale, (data['x'][e[0]:e[-1]+1] - np.expand_dims(_drange[:, 0], 0)))
                        if unit == 'eV':
                            _dx[e[0]:e[-1]+1] = np.einsum('k,ijkl->ijkl', scale, data['dxdr'][e[0]:e[-1]+1])
                            _rdx[e[0]:e[-1]+1] = np.einsum('j,ijk->ijk', scale, data['rdxdr'][e[0]:e[-1]+1])
                        else:
                            _dx[e[0]:e[-1]+1] = 0.529177*np.einsum('k,ijkl->ijkl', scale, data['dxdr'][e[0]:e[-1]+1])
                            _rdx[e[0]:e[-1]+1] = 0.529177*np.einsum('j,ijk->ijk', scale, data['rdxdr'][e[0]:e[-1]+1])

                x[0:shp[0]] = _x
                dxdr[0:shp[0], 0:shp[0]] = _dx
                rdxdr[0:shp[0]] = _rdx
                force[0:shp[0]] = data['force']
                ...
                if (i+1)%bundle == 0 or (i+1)==self.no_of_structures:
                    np.save(self.path+'train_npys/X'+str(count_bundle)+'.npy', X)
                    np.save(self.path+'train_npys/DX'+str(count_bundle)+'.npy', DXDR)
                    np.save(self.path+'train_npys/RDX'+str(count_bundle)+'.npy', RDXDR)
                    np.save(self.path+'train_npys/APS'+str(count_bundle)+'.npy', APS)
                    np.save(self.path+'train_npys/ENERGY'+str(count_bundle)+'.npy', ENERGY)
                    np.save(self.path+'train_npys/FORCES'+str(count_bundle)+'.npy', FORCES)
                    np.save(self.path+'train_npys/STRESS'+str(count_bundle)+'.npy', STRESS)
                    np.save(self.path+'train_npys/GROUP'+str(count_bundle)+'.npy', GROUP)
    ...
    def _SOFTMAX(self, data, beta=-1):
        ...
        if beta is not None:
            ...
            d = db[str(i)]
            for i in range(self.no_of_structures):
                _epa = d['energy'][0]/len(d['elements'])
                epa.append(_epa)
...
class Dataset(data.Dataset):
    ...
    """
    def __init__(self, path, atoms, softmax, device, memory, fc, sc, length):
        self.path = path
        self.atoms = atoms
        ...
        self.length = length
        self.bundle = 1000
        self.bundles = length // self.bundle + 1
        print(self.bundles)
        self.temp_i = None
        self._energy, self._force, self._stress = None, None, None
        self._x, self._dx, self._rdx = None, None , None
        self._aps, self._group = None, None


        if memory == 'in':
            self.x, self.dxdr, self.rdxdr, self.aps = [], [], [], []
            self.energy, self.force, self.stress, self.group = [], [], [], []

            for i in range(self.bundles):
                _aps = np.load(self.path+'train_npys/APS'+str(i+1)+'.npy')
                _group = np.load(self.path+'train_npys/GROUP'+str(i+1)+'.npy')
                _energy = np.load(self.path+'train_npys/ENERGY'+str(i+1)+'.npy')
                _x = np.load(self.path+'train_npys/X'+str(i+1)+'.npy')
                
                if self.fc:
                    _force = np.load(self.path+'train_npys/FORCES'+str(i+1)+'.npy')
                    _dx = np.load(self.path+'train_npys/DX'+str(i+1)+'.npy')
                if self.sc:
                    _stress = np.load(self.path+'train_npys/STRESS'+str(i+1)+'.npy')
                    _rdx = np.load(self.path+'train_npys/RDX'+str(i+1)+'.npy')
                else:
                    _stress = [None]*self.bundle 
               
                for j in range(len(_energy)):
                    self.aps.append(_aps[j])
                    self.group.append(_group[j])
                    self.energy.append(_energy[j])
                    self.x.append(torch.from_numpy(_x[j][0:self.atoms[self.bundle*i+j]]).to(self.device))

                    if self.fc:
                        self.force.append(torch.from_numpy(_force[j][0:self.atoms[self.bundle*i+j]]).to(self.device))
                        self.dxdr.append(torch.from_numpy(_dx[j][0:self.atoms[self.bundle*i+j], 
                                                                 0:self.atoms[self.bundle*i+j]]).to(self.device))
                    else:
                        self.force.append(None)
                        self.dxdr.append(None)

                    if self.sc and _stress[j] is not None:
                        self.stress.append(torch.from_numpy(_stress[j]).to(self.device))
                        self.rdxdr.append(torch.from_numpy(_rdx[j][0:self.atoms[self.bundle*i+j]]).to(self.device))
                    else:
            ...
            self._APS, self._GROUP = [], []
            self._ENERGY, self._FORCE, self._STRESS = [], [], []
            self._X, self._DX, self._RDX = [], [], []

            for i in range(self.bundles-1):
                self._APS.append(np.load(self.path+'train_npys/APS'+str(i+1)+'.npy', mmap_mode='r'))
                self._GROUP.append(np.load(self.path+'train_npys/GROUP'+str(i+1)+'.npy', mmap_mode='r'))
                self._ENERGY.append(np.load(self.path+'train_npys/ENERGY'+str(i+1)+'.npy', mmap_mode='r'))
                self._X.append(np.load(self.path+'train_npys/X'+str(i+1)+'.npy', mmap_mode='r'))
                if self.fc:
                    self._FORCE.append(np.load(self.path+'train_npys/FORCES'+str(i+1)+'.npy', mmap_mode='r'))
                    self._DX.append(np.load(self.path+'train_npys/DX'+str(i+1)+'.npy', mmap_mode='r'))
                if self.sc:
                    self._STRESS.append(np.load(self.path+'train_npys/STRESS'+str(i+1)+'.npy', mmap_mode='r'))
                    self._RDX.append(np.load(self.path+'train_npys/RDX'+str(i+1)+'.npy', mmap_mode='r'))
                else:
                    self._STRESS.append([None]*self.bundle)
    ...
    def __getitem__(self, index):
        ...
        if self.memory == 'in':
            aps = self.aps[index]       # atom per specie
            ...
            #_APS = np.load(self.path+'train_npys/APS'+str(i+1)+'.npy', mmap_mode='c')
            #_GROUP = np.load(self.path+'train_npys/GROUP'+str(i+1)+'.npy', mmap_mode='c')
            #_ENERGY = np.load(self.path+'train_npys/ENERGY'+str(i+1)+'.npy', mmap_mode='c')
            #_X = np.load(self.path+'train_npys/X'+str(i+1)+'.npy', mmap_mode='c')
            #if self.fc:
            #    _FORCE = np.load(self.path+'train_npys/FORCES'+str(i+1)+'.npy', mmap_mode='c')
            #    _DX = np.load(self.path+'train_npys/DX'+str(i+1)+'.npy', mmap_mode='c')
            #if self.sc:
            #    _STRESS = np.load(self.path+'train_npys/STRESS'+str(i+1)+'.npy', mmap_mode='c')
            #    _RDX = np.load(self.path+'train_npys/RDX'+str(i+1)+'.npy', mmap_mode='c')
            #else:
            #    _STRESS = [None]*self.bundle

            #_aps = deepcopy(_APS[j])
            #_group = deepcopy(_GROUP[j])
            #_energy = deepcopy(_ENERGY[j])
            #_x = deepcopy(_X[j][0:atoms])
            #_force = deepcopy(_FORCE[j][:atoms])
            #_dx = deepcopy(_DX[j][0:atoms, 0:atoms])
            #if self.sc:
            #    _stress = deepcopy(_STRESS[j])
            #    _rdx = deepcopy(_RDX[j][0:atoms])
            #else:
            #    _stress = _STRESS[j]
            
            aps = self._APS[i][j]
            group = self._GROUP[i][j]
            energy = torch.from_numpy(np.array([self._ENERGY[i][j]])).to(self.device)[0]
            x = torch.from_numpy(self._X[i][j][0:atoms]).to(self.device)
            force = torch.from_numpy(self._FORCE[i][j][0:atoms]).to(self.device) if self.fc else None
            dxdr = torch.from_numpy(self._DX[i][j][0:atoms, 0:atoms]).to(self.device) if self.fc else None
            stress = torch.from_numpy(self._STRESS[i][j]).to(self.device) if (self.sc and self._STRESS[i][j] is not None) else None
            rdxdr = torch.from_numpy(self._RDX[i][j][0:atoms]).to(self.device) if (self.sc and self._STRESS[i][j] is not None) else None
                        
        return x, dxdr, rdxdr, energy, force, stress, sf, group, aps, atoms
